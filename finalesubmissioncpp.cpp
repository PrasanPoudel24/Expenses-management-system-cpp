          ...
          switch (select) {
		   ...
		   void edit() 
		   {
				    ...
				    if (expenses.find(cet) != expenses.end())
					 {
				        int value;
				        cout << "Enter how much amount you have saved already: ";
				        cin >> value;
				
				        
				        if (value == expenses[cet]) 
						{
				            expenses.erase(cet);
				            dis("Category deleted successfully!");
						...
						{
				            cout << "Amount does not match. Deletion failed." << endl;
				        }
				    } 
					else
					{
